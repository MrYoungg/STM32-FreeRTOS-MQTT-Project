...
typedef enum mqtt_qos { QOS0 = 0, QOS1 = 1, QOS2 = 2, SUBFAIL = 0x80 } mqtt_qos_t;
...
dcl_class(mqtt_connack_data_t) def_class(mqtt_connack_data_t, private_member(uint8_t rc; uint8_t session_present;))
    dcl_class(message_handlers_t) def_class(message_handlers_t,
        ...
        def_class(ack_handlers_t,
                  private_member(mqtt_list_t list; platform_timer_t timer; uint32_t type; uint16_t packet_id;
                                 message_handlers_t * handler;
                                 uint16_t payload_len;
                                 uint8_t * payload;))

            dcl_class(mqtt_will_options_t) def_class(mqtt_will_options_t,
                                                     private_member(mqtt_qos_t will_qos; uint8_t will_retained;
                                                                    char *will_topic;
                                                                    char *will_message;))

                dcl_class(mqtt_client_t) def_class(mqtt_client_t,
                                                   private_member(char *mqtt_client_id; char *mqtt_user_name;
                                                                  char *mqtt_password;
                                                                  char *mqtt_host;
                                                                  char *mqtt_port;
                                                                  char *mqtt_ca;
...
// MQTT客户端配置函数的声明
#define MQTT_CLIENT_SET_STATEMENT(name, type) type mqtt_set_##name(mqtt_client_t *, type);

                    MQTT_CLIENT_SET_STATEMENT(client_id, char *) MQTT_CLIENT_SET_STATEMENT(user_name, char *)
                        MQTT_CLIENT_SET_STATEMENT(password, char *) MQTT_CLIENT_SET_STATEMENT(host, char *)
                            MQTT_CLIENT_SET_STATEMENT(port, char *) MQTT_CLIENT_SET_STATEMENT(ca, char *)
                                MQTT_CLIENT_SET_STATEMENT(reconnect_data, void *)
                                    MQTT_CLIENT_SET_STATEMENT(keep_alive_interval,
                                                              uint16_t) MQTT_CLIENT_SET_STATEMENT(will_flag, uint32_t)
                                                    ...
                                                    MQTT_CLIENT_SET_STATEMENT(read_buf_size, uint32_t)
                                                        MQTT_CLIENT_SET_STATEMENT(write_buf_size, uint32_t)
                                                            MQTT_CLIENT_SET_STATEMENT(reconnect_try_duration, uint32_t)
                                                                MQTT_CLIENT_SET_STATEMENT(reconnect_handler,
                                                                                          reconnect_handler_t)
                                                                    MQTT_CLIENT_SET_STATEMENT(interceptor_handler,
