...
static client_state_t mqtt_get_client_state(mqtt_client_t *c) { return c->mqtt_client_state; }
...
static int mqtt_read_packet(mqtt_client_t *c, int *packet_type, platform_timer_t *timer)
{
    ...
    if ((remain_len > 0) && ((rc = network_read(c->mqtt_network, c->mqtt_read_buf + len, remain_len,
...
static void mqtt_new_message_data(message_data_t *md, MQTTString *topic_name,
                                  mqtt_message_t *message)
{
    int len;
    len = (topic_name->lenstring.len < MQTT_TOPIC_LEN_MAX - 1) ? topic_name->lenstring.len
...
static message_handlers_t *mqtt_get_msg_handler(mqtt_client_t *c, MQTTString *topic_name)
{
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list)
    {
        ...
        if ((NULL != msg_handler->topic_filter) &&
            ((MQTTPacket_equals(topic_name, (char *)msg_handler->topic_filter)) ||
...
static int mqtt_deliver_message(mqtt_client_t *c, MQTTString *topic_name, mqtt_message_t *message)
{
    ...
    if (NULL != msg_handler) {
        ...
        msg_handler->handler(c, &md); /* deliver the message, call the handler function */
...
static ack_handlers_t *mqtt_ack_handler_create(mqtt_client_t *c, int type, uint16_t packet_id,
                                               uint16_t payload_len, message_handlers_t *handler)
{
    ...
    platform_timer_cutdown(
        &ack_handler->timer,
        c->mqtt_cmd_timeout); /* No response within timeout will be destroyed or resent */
...
static void mqtt_ack_handler_resend(mqtt_client_t *c, ack_handlers_t *ack_handler)
{
    ...
    memcpy(c->mqtt_write_buf, ack_handler->payload,
    ...
    MQTT_LOG_W("%s:%d %s()... resend %d package, packet_id is %d ", __FILE__, __LINE__,
               __FUNCTION__, ack_handler->type, ack_handler->packet_id);
}
...
static int mqtt_ack_list_record(mqtt_client_t *c, int type, uint16_t packet_id,
                                uint16_t payload_len, message_handlers_t *handler)
{
    int rc = MQTT_SUCCESS_ERROR;
    ack_handlers_t *ack_handler = NULL;
...
static int mqtt_ack_list_unrecord(mqtt_client_t *c, int type, uint16_t packet_id,
...
static message_handlers_t *mqtt_msg_handler_create(const char *topic_filter, mqtt_qos_t qos,
...
static int mqtt_msg_handler_is_exist(mqtt_client_t *c, message_handlers_t *handler)
{
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list)
    {
        ...
        if ((NULL != msg_handler->topic_filter) &&
            (mqtt_is_topic_equals(msg_handler->topic_filter, handler->topic_filter))) {
...
/**
 * see if there is a message waiting for the server to answer in the ack list, if there is, then
 * process it according to the flag. flag : 0 means it does not need to wait for the timeout to
 * process these packets immediately. usually immediately after reconnecting. 1 means it needs to
 * wait for timeout before processing these messages, usually timeout processing in a stable
 * connection.
 */
...
static int mqtt_try_resubscribe(mqtt_client_t *c)
{
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list)
    {
        ...
        if ((rc = mqtt_subscribe(c, msg_handler->topic_filter, msg_handler->qos,
                                 msg_handler->handler)) == MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR)
            MQTT_LOG_W("%s:%d %s()... mqtt ack handler num too much ...", __FILE__, __LINE__,
                       __FUNCTION__);
    }
...
static int mqtt_try_do_reconnect(mqtt_client_t *c)
{
    ...
    MQTT_LOG_D("%s:%d %s()... mqtt try connect result is -0x%04x", __FILE__, __LINE__, __FUNCTION__,
               -rc);
...
static int mqtt_try_reconnect(mqtt_client_t *c)
{
    ...
    /*before connect, call reconnect handler, it can used to update the mqtt password, eg: onenet
     * platform need*/
    ...
    if (MQTT_SUCCESS_ERROR != rc) {
        /*connect fail must delay reconnect try duration time and let cpu time go out, the lowest
         * priority task can run */
...
static int mqtt_publish_ack_packet(mqtt_client_t *c, uint16_t packet_id, int packet_type)
{
    ...
    switch (packet_type) {
    case PUBREC:
        len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBREL, 0,
                                packet_id); /* make a PUBREL ack packet */
        rc = mqtt_ack_list_record(c, PUBCOMP, packet_id, len,
    ...
    case PUBREL:
        len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBCOMP, 0,
...
static int mqtt_suback_packet_handle(mqtt_client_t *c, platform_timer_t *timer)
{
    ...
    if (MQTTDeserialize_suback(&packet_id, 1, &count, (int *)&granted_qos, c->mqtt_read_buf,
                               c->mqtt_read_buf_size) != 1)
...
static int mqtt_unsuback_packet_handle(mqtt_client_t *c, platform_timer_t *timer)
{
    ...
    rc = mqtt_ack_list_unrecord(c, UNSUBACK, packet_id,
...
static int mqtt_publish_packet_handle(mqtt_client_t *c, platform_timer_t *timer)
{
    ...
    if (MQTTDeserialize_publish(&msg.dup, &qos, &msg.retained, &msg.id, &topic_name,
                                (uint8_t **)&msg.payload, (int *)&msg.payloadlen, c->mqtt_read_buf,
                                c->mqtt_read_buf_size) != 1)
    ...
    if (msg.qos != QOS2)
        ...
        /* record the received of a qos2 message and only processes it when the qos2 message is
...
static int mqtt_packet_handle(mqtt_client_t *c, platform_timer_t *timer)
{
    ...
    case 0: /* timed out reading packet or an error occurred while reading data*/
        if (MQTT_BUFFER_TOO_SHORT_ERROR == rc) {
            MQTT_LOG_E("the client read buffer is too short, please call mqtt_set_read_buf_size() "
                       "to reset the buffer size");
            /* don't return directly, you need to stay active, because there is data readable now,
             * but the buffer is too small */
        }
        break;
...
static void mqtt_yield_thread(void *arg)
{
    ...
    if (CLIENT_STATE_CONNECTED != state) {
        MQTT_LOG_W("%s:%d %s()..., mqtt is not connected to the server...", __FILE__, __LINE__,
                   __FUNCTION__);
    ...
    while (1) {
        ...
        if (MQTT_CLEAN_SESSION_ERROR == rc) {
            ...
            MQTT_LOG_W("%s:%d %s()..., mqtt reconnect timeout....", __FILE__, __LINE__,
                       __FUNCTION__);
        }
    }
...
static int mqtt_connect_with_results(mqtt_client_t *c)
{
    ...
    if ((len = MQTTSerialize_connect(c->mqtt_write_buf, c->mqtt_write_buf_size, &connect_data)) <=
        0)
        goto exit;
...
exit:
    if (rc == MQTT_SUCCESS_ERROR) {
        if (NULL == c->mqtt_thread) {
            ...
            c->mqtt_thread =
                platform_thread_init("mqtt_yield_thread", mqtt_yield_thread, c,
            ...
            if (NULL != c->mqtt_thread) {
                ...
                network_release(c->mqtt_network);
                rc = MQTT_CONNECT_FAILED_ERROR;
                MQTT_LOG_W("%s:%d %s()... mqtt yield thread creat faile...", __FILE__, __LINE__,
                           __FUNCTION__);
            }
        }
        else {
            mqtt_set_client_state(
                c, CLIENT_STATE_CONNECTED); /* reconnect, mqtt thread is already exists */
        }
...
void mqtt_sleep_ms(int ms) { platform_timer_usleep(ms * 1000); }
...
int mqtt_keep_alive(mqtt_client_t *c)
{
    ...
    if (platform_timer_is_expired(&c->mqtt_last_sent) ||
        ...
        if (c->mqtt_ping_outstanding) {
            ...
            if (len > 0 && (rc = mqtt_send_packet(c, len, &timer)) ==
                               MQTT_SUCCESS_ERROR) // send the ping packet
...
int mqtt_subscribe(mqtt_client_t *c, const char *topic_filter, mqtt_qos_t qos,
                   message_handler_t handler)
{
    ...
    len = MQTTSerialize_subscribe(c->mqtt_write_buf, c->mqtt_write_buf_size, 0, packet_id, 1,
                                  &topic, (int *)&qos);
    if (len <= 0)
        goto exit;

    /* send the serialize subscribe packet */
    if ((rc = mqtt_send_packet(c, len, &timer)) != MQTT_SUCCESS_ERROR)
        goto exit;

    if (NULL == handler)
        handler =
            default_msg_handler; /* if handler is not specified, the default handler is used */

    /* create a message and record it(add topic to list) */
...
int mqtt_unsubscribe(mqtt_client_t *c, const char *topic_filter)
{
    ...
    if ((len = MQTTSerialize_unsubscribe(c->mqtt_write_buf, c->mqtt_write_buf_size, 0, packet_id, 1,
                                         &topic)) <= 0)
        goto exit;
    if ((rc = mqtt_send_packet(c, len, &timer)) != MQTT_SUCCESS_ERROR)
        goto exit;
...
int mqtt_publish(mqtt_client_t *c, const char *topic_filter, mqtt_message_t *msg)
{
    ...
    if (QOS0 != msg->qos) {
        if (mqtt_ack_handler_is_maximum(c)) {
            rc = MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR; /* the recorded ack handler has reached the
                                                         maximum */
            goto exit;
        }
    ...
    len =
        MQTTSerialize_publish(c->mqtt_write_buf, c->mqtt_write_buf_size, 0, msg->qos, msg->retained,
                              msg->id, topic, (uint8_t *)msg->payload, msg->payloadlen);
    if (len <= 0)
        goto exit;
...
exit:
    ...
    if ((MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR == rc) || (MQTT_MEM_NOT_ENOUGH_ERROR == rc)) {
        MQTT_LOG_W("%s:%d %s()... there is not enough memory space to record...", __FILE__,
...
int mqtt_list_subscribe_topic(mqtt_client_t *c)
{
    ...
    if (mqtt_list_is_empty(&c->mqtt_msg_handler_list))
        MQTT_LOG_I("%s:%d %s()... there are no subscribed topics...", __FILE__, __LINE__,
                   __FUNCTION__);
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list)
    {
        ...
        /* determine whether a node already exists by mqtt topic, but wildcards are not supported */
        if (NULL != msg_handler->topic_filter) {
            MQTT_LOG_I("%s:%d %s()...[%d] subscribe topic: %s", __FILE__, __LINE__, __FUNCTION__,
...
int mqtt_set_will_options(mqtt_client_t *c, char *topic, mqtt_qos_t qos, uint8_t retained,
                          char *message)
{
    ...
    if (NULL == c->mqtt_will_options) {
        c->mqtt_will_options =
            (mqtt_will_options_t *)platform_memory_alloc(sizeof(mqtt_will_options_t));
