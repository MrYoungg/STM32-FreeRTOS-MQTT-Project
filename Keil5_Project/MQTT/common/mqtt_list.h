...
#define LIST_NODE(node) {&(node), &(node)}

#define LIST_DEFINE(list) mqtt_list_t list = {&(list), &(list)}

#define LIST_ENTRY(list, type, field) CONTAINER_OF_FIELD(list, type, field)

#define LIST_FIRST_ENTRY(list, type, field) LIST_ENTRY((list)->next, type, field)

#define LIST_FIRST_ENTRY_OR_NULL(list, type, field)                                                \
    (mqtt_list_is_empty(list) ? NULL : LIST_FIRST_ENTRY(list, type, field))

#define LIST_FOR_EACH(curr, list) for (curr = (list)->next; curr != (list); curr = curr->next)

#define LIST_FOR_EACH_PREV(curr, list) for (curr = (list)->prev; curr != (list); curr = curr->prev)
