...
static void ExFlash_Menu(void)
{
    ...
    DEBUG_LOG("当前外存存储情况如下： \r\n");
    DEBUG_LOG("|%6s|%28s|%10s|%16s|\r\n", Area, Version, ProgSize, Remark);
...
select:
    ...
    if (ExFlash_Num <= 0 || ExFlash_Num >= EXFLASH_APP_NUM) {
        if (ExFlash_Num == 0) {
            DEBUG_LOG("0号存储块仅供OTA使用，请重新输入（1~%d） \r\n", EXFLASH_APP_NUM - 1);
            goto select;
        }
        DEBUG_LOG("请输入正确的外存编号（1~%d） \r\n", EXFLASH_APP_NUM - 1);
        goto select;
    }
...
void ExFlash_MoveInAPP(void)
{
    ...
    // 2、选择要读出的外存块
    DEBUG_LOG("请选择要读取的外存块：1~%d \r\n", EXFLASH_APP_NUM - 1);
reselect:
    ExFlash_Num = ExFlash_SelectBlock();
    info = &(OTA_Info.ExFlash_InfoArr[ExFlash_Num]);

    if (info->size == 0) {
        DEBUG_LOG("当前存储块没有程序，请重新输入 \r\n");
        goto reselect;
    }
    else if (info->size > APP_MAX_SIZE) {
        DEBUG_LOG("当前存储块程序过大，请重新选择 \r\n");
        goto reselect;
    }

    DEBUG_LOG("准备将%d号外存块写入内部Flash \r\n", ExFlash_Num);
