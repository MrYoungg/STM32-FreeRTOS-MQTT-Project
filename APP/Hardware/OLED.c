...
void OLED_ShowNum(uint8_t X, uint8_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    for (i = 0; i < Length; i++) // 遍历数字的每一位
    {
        ...
        /*+ '0' 可将数字转换为字符格式*/
        OLED_ShowChar(
            X + i * FontSize, Y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
    }
}
...
void OLED_ShowSignedNum(uint8_t X, uint8_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    for (i = 0; i < Length; i++) // 遍历数字的每一位
    {
        ...
        /*+ '0' 可将数字转换为字符格式*/
        OLED_ShowChar(
            X + (i + 1) * FontSize, Y, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
    }
}
...
void OLED_ShowBinNum(uint8_t X, uint8_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    for (i = 0; i < Length; i++) // 遍历数字的每一位
    {
        ...
        /*+ '0' 可将数字转换为字符格式*/
        OLED_ShowChar(
            X + i * FontSize, Y, Number / OLED_Pow(2, Length - i - 1) % 2 + '0', FontSize);
    }
}
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void OLED_ShowFloatNum(
    uint8_t X, uint8_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
    uint32_t Temp;
    ...
    OLED_ShowNum(
        X + (IntLength + 2) * FontSize, Y, ((uint32_t)(Number * Temp)) % Temp, FraLength, FontSize);
}
...
void OLED_ShowChinese(uint8_t X, uint8_t Y, char *Chinese)
{
    ...
    for (i = 0; Chinese[i] != '\0'; i++) // 遍历汉字串
    {
        ...
        if (pChinese >= OLED_CHN_CHAR_WIDTH) {
            ...
            OLED_ShowImage(
                X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 16, OLED_CF16x16[pIndex].Data);
        }
    }
}
...
void OLED_DrawTriangle(
    uint8_t X0, uint8_t Y0, uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2, uint8_t IsFilled)
{
 ...
 */
void OLED_DrawArc(
    uint8_t X, uint8_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
    int16_t x, y, d, j;
