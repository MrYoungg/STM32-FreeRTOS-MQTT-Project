...
int Write_RingBuffer(RingBuffer_t *ringBuffer, const uint8_t *data, uint32_t dataLength)
{
    ...
    if (dataLength > (ringBuffer->bufferSize - ringBuffer->dataSize)) {
        DEBUG_LOG("Ring buffer is not enough\r\n");
        return false;
    }

    // 写入数据（必须逐个写，因为可能经过末尾回到开头）
...
{
    FCB_ListItem_t *Item = (FCB_ListItem_t *)pvPortMalloc(sizeof(FCB_ListItem_t));
    if (Item == NULL) {
        return NULL;
    }
...
{
    return (ringBuffer->FCBListSize == 0);
}
